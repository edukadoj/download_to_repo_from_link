...
def execute_one_command(
    ...
    # ── Other basic commands ──
        ...
        if plain is None: result = "ERR decode"
            ...
            try:
                ...
                if elem is None:
                    result = "ERR decode: no focused element"
                else:
        ...
        if plain is None: result = "ERR humantype"
            ...
            try:
                elem = driver.switch_to.active_element
                if elem is None:
                    result = "ERR humantype: no focused element"
                else:
                    for ch in plain:
                        elem.send_keys(ch)
        ...
        try:
            ...
            main_mod = sys.modules.get("__main__")
            if main_mod:
                main_mod.LAST_NAVIGATED_URL = current_url
        except Exception:
            pass
    elif cmd == "drag":
        agent_state.ensure_active_tab()
    ...
    # ── Zoom command ──
        ...
        try:
            factor = float(arg)
            driver.execute_script("document.body.style.zoom = arguments[0]", factor)
            result = f"OK zoom({factor})"
        except Exception as e:
            result = f"ERR zoom: {e}"

    # ── download command (per‑chunk via upload_file_direct) ──
    elif cmd == "download":
        selected = [os.path.join(DOWNLOAD_DIR, f) for f in _upload_file_paths if os.path.isfile(os.path.join(DOWNLOAD_DIR, f))]
        if not selected:
            result = "ERR download: no selected files"
        elif not rw:
            result = "ERR download: RepoWrapper not available"
        else:
            ...
            for fpath in selected:
                ...
                with tempfile.TemporaryDirectory() as tmpdir:
                    proc = subprocess.run(
                        ["python3", _CHUNKER_PATH, "--file", fpath, "--output-dir", tmpdir, "--chunk-size", "20"],
                        capture_output=True, text=True
                    )
                    if proc.returncode != 0:
                        err = proc.stderr.strip() or proc.stdout.strip() or "unknown error"
                        result = f"ERR download: chunker failed for {fname}: {err}"
                        success = False
                        break

                    part_files = sorted(glob.glob(os.path.join(tmpdir, "*.part*")))
                    for idx, part_file in enumerate(part_files, 1):
                        chunk_name = os.path.basename(part_file)
                        repo_rel = f"AgentDownload/{chunk_name}"
                        with open(part_file, "rb") as pf:
                            chunk_content = pf.read()
                        if rw.upload_file_direct(repo_rel, chunk_content):
                            add_autonomous_report("download-progress",
                                                  f"Chunk {idx}/{len(part_files)} of {fname} uploaded")
                        else:
                            result = f"ERR download: failed to upload chunk {chunk_name}"
                            success = False
                            break
                    if not success:
                        break
            if success:
                result = f"OK download({total_files} files)"
        ...
        result = perform_upload(
            ...
            _file_registry, _upload_file_paths,
            git_push_with_retry, inject_file,
            log_func=log_func,
            rw=rw
        )
        _ensure_selection(_file_registry, _upload_file_paths)
    ...
    # ── Tab / dir commands ──
        ...
        try:
            ...
            if ids:
                ...
                _upload_file_paths.clear()
                add_autonomous_report("selectfiles", "selectfiles()")
                result = "Upload file(s) cleared (no selection)"
        except Exception: result = "ERR invalid upload numbers"
    elif cmd == "savestate":
        ok, msg = save_profile(chunk_size_mb=20)
