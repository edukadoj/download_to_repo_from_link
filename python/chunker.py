#!/usr/bin/env python3
# Version: 3.3 – 1 MB = 1,000,000 bytes (safe under GitHub's Content API limit)
"""
chunker.py - Split files into 1,000,000‑byte chunks and generate a
reassemble.bat that works for multiple files.
...
Outputs:
    ...
    - reassemble.bat – loops over all .part0000 files and reassembles each
"""
...
DEFAULT_CHUNK_MB = 1          # 1 MB = 1,000,000 bytes
BYTES_PER_MB   = 1_000_000   # safe under GitHub's 1 MB API limit
...
def split_file(input_path: Path, output_dir: Path, chunk_size: int, base_name: str = None):
    ...
    print(f"Splitting {input_path} ({file_size} bytes) into {num_chunks} chunk(s) of {chunk_size} bytes each")
...
def main():
    parser = argparse.ArgumentParser(description="Split files into 1,000,000‑byte chunks and generate reassembly batch file")
    ...
    parser.add_argument('--chunk-size', type=int, default=DEFAULT_CHUNK_MB,
                        help=f'Chunk size in MB (1 MB = {BYTES_PER_MB} bytes, default {DEFAULT_CHUNK_MB})')
    ...
    chunk_size_bytes = args.chunk_size * BYTES_PER_MB
    ...
    for input_path in input_paths:
        ...
        split_file(input_path, output_dir, chunk_size_bytes, base_name)
