...
def _driver_health_check(driver, driver_lock, timeout=3):
    """Non‑blocking health check – executes a trivial script."""
    def check():
        try:
            with driver_lock:
...
class ScreenshotWorker:
    def __init__(self, stop_event, driver, driver_lock, agent_state,
        ...
        self._sync_repo = sync_repo          # SyncRepo instance
        self._rw = sync_repo._rw             # RepoWrapper instance
        self._log = log_func
        self._push_logs = push_logs_func     # just enqueues log via sync_repo.push_log_file()
    ...
    def _take_screenshot(self, desc="auto", push=False):
        ...
        filename = f"screenshots/{self._counter:04d}_{ts_us}_{desc}.png"
        ...
        if push:
            self._log(f"Uploading screenshot synchronously: {filename}")
            ...
            try:
                ...
                while not self._stop.is_set():
                    ...
                    if filename is None:
                        ...
                        # Push the log via the background pusher
                        self._push_logs()
                        interval = max(0, self._get_comm_interval() * self._get_slow_mode() - (time.time() - start))
                        self._stop.wait(interval)
                        continue

                    # Push the log via the background pusher (enqueues, safe)
                    self._push_logs()
