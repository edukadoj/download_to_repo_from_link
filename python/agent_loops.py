...
from profile_cache import save_profile as profile_save
...
FETCHER_POLL_INTERVAL = 5.0

# ── In‑progress command tracking (prevents duplicate error reports) ──
        ...
        try:
            if app_cmd_id:
                ...
                if body_hash == last_app_body_hash:
                    _fetcher_stop.wait(FETCHER_POLL_INTERVAL)
                    continue
                ...
                for line in lines:
                    ...
                    # ── FIRST: check if already being processed ──
                    with _in_progress_lock:
                        if cid in _in_progress_ids:
                            safe_log(f"Skipping duplicate in‑progress command {cid}")
                            continue

                    # ── SECOND: check report_history (duplicate command ID) ──
                    ...
                    # ── NOW: check locks for genuinely new commands ──
                    if cmd_type in ("save", "upload", "download"):
                    ...
                    if cmd_type in ("upload", "uploadtoyoutube") and upload_lock.locked():
                        ...
                        continue
                    if cmd_type == "download" and upload_lock.locked():
                        ts = int(time.time() * 1_000_000)
                        seq_num = 0
                        if cid.startswith("APP-"):
                            parts_cid = cid.split('-')
                        ...
                        error_result = "ERR download already in progress"
                        with _report_history_lock:
                            report_history[cid] = (ts, seq_num, error_result)
                        save_report_history()
                        add_to_report_queue(cid, ts, seq_num, error_result)
                        safe_log(f"Rejected duplicate download command {cid}")
                        continue

                    # Mark as in‑progress
                    with _in_progress_lock:
                        _in_progress_ids.add(cid)
...
# ---------- Light Execution Loop (15-second timeout) ----------
def executor_loop(
        ...
        def run_command():
            try:
                ...
                return execute_one_command(
                    ...
                    git_push_with_retry=(lambda: True),
        ...
        def run_command():
            ...
            try:
                if cmd_type == "save":
                    save_lock.acquire()
                    lock_acquired = True
                elif cmd_type in ("upload", "uploadtoyoutube", "download"):
                    upload_lock.acquire()
                    lock_acquired = True

                if cmd_type == "save":
                    ok, msg = profile_save(
                        CACHE_DIR, PROFILE_DIR, ENCRYPTION_KEY_PROFILE,
                        REPO, PAT, rw, CHUNK_SIZE_MB
                    )
