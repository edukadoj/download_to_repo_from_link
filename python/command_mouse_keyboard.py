#!/usr/bin/env python3
# ==============================================================================
# command_mouse_keyboard.py – Version 39.39.13
#   - After a profile cache is loaded successfully, other older/unhealthy
#     cache directories are purged.
# ==============================================================================
...
from agent_state import (
    ...
    start_tab_monitor,
    set_autonomous_report_callback,
    download_folder_watcher,
    _download_watcher_stop
)
...
echo(f"{'='*60}\n  Remote Control v39.39.13 started at {datetime.now().strftime('%Y-%m-%d %H:%M:%S UTC')}\n{'='*60}")
...
# Global to remember the last navigated URL for restart recovery
LAST_NAVIGATED_URL = ""
...
# ---------- Autonomous report callback ----------
def _autonomous_callback(report_type, text):
    add_autonomous_report(report_type, text)

# ---------- Purge other cache directories after a successful load ----------
def purge_other_cache_dirs(keep_dir_name: str):
    """Remove all timestamped cache directories except the one specified."""
    if not os.path.isdir(CACHE_DIR):
        return
    for name in os.listdir(CACHE_DIR):
        full = os.path.join(CACHE_DIR, name)
        if os.path.isdir(full) and re.match(r'^\d{8}_\d{6}$', name) and name != keep_dir_name:
            shutil.rmtree(full, ignore_errors=True)
            safe_log(f"Purged old cache directory: {name}")

# ---------- Download latest profile cache from repo ----------
def _download_latest_profile_cache():
    """Download the newest timestamped cache folder from the repository."""
    safe_log("Downloading latest profile cache from repo...")
    event = threading.Event()
    listing = []
    def cb(lst):
        listing.extend(lst)
        event.set()
    repo_wrapper.list_directory(CACHE_DIR, cb)
    if not event.wait(timeout=30):
        safe_log("Timeout listing profile cache directory – proceeding without cache.")
        return
    ...
    for path, sha in listing:
        ...
        if len(parts) >= 3:
            ts_dir = parts[1]
            if re.match(r'^\d{8}_\d{6}$', ts_dir):
                subdirs.setdefault(ts_dir, []).append((path, sha))

    if not subdirs:
        safe_log("No profile cache folders found in repo.")
        return

    newest = max(subdirs.keys())
    safe_log(f"Newest cache folder: {newest}")
    local_dir = os.path.join(CACHE_DIR, newest)
    os.makedirs(local_dir, exist_ok=True)

    for path, sha in subdirs[newest]:
        event.clear()
        data_holder = []
        def download_cb(data):
            data_holder.append(data)
            event.set()
        repo_wrapper.download_file(path, download_cb)
        if event.wait(timeout=30) and data_holder:
            local_path = os.path.join(local_dir, os.path.basename(path))
            with open(local_path, 'wb') as f:
                f.write(data_holder[0])
            safe_log(f"  Downloaded {os.path.basename(path)} ({len(data_holder[0])} bytes)")
        else:
            safe_log(f"  Failed to download {path}")

    safe_log(f"Profile cache download finished – {len(subdirs[newest])} files.")
...
# ---------- Browser restart helper (now recovers URL) ----------
def restart_browser():
    global driver, LAST_NAVIGATED_URL
    ...
    try:
        ...
        # ── Recover the last navigated URL ───────────────────────────
        recover_url = LAST_NAVIGATED_URL or START_URL
        safe_log(f"Navigating to recovered URL: {recover_url}")
        driver.get(recover_url)

        # Re‑apply CDP download behavior
        try:
            from agent_state import _cdp_send
            _cdp_send("Page.setDownloadBehavior", {
                "behavior": "allow",
...
# ──────────────────────────────────────────────────────────────────
#  EARLY INITIALISATION – wrapped so any failure is logged
# ──────────────────────────────────────────────────────────────────
try:
    ...
    # Download latest cache before trying to load it
    _download_latest_profile_cache()

    # Attempt to load a profile cache; if Chrome fails, delete the corrupt cache and retry
    driver = None
    profile_loaded = False
    loaded_cache_dir = None
    while True:
        profile_loaded, loaded_cache_dir = load_profile(CACHE_DIR, PROFILE_DIR, ENCRYPTION_KEY, REPO,
        ...
        try:
            ...
            try:
                ...
                safe_log("CDP download behavior set to allow (silent).")
            except Exception as e:
                safe_log(f"Could not set CDP download behavior: {e}")
            ...
            # ── If a profile was loaded successfully, purge other cache dirs ──
            if profile_loaded and loaded_cache_dir is not None:
                purge_other_cache_dirs(loaded_cache_dir)
    ...
    # ── Wire autonomous reports to the sender queue ───────────────
    set_autonomous_report_callback(add_to_report_queue)

    # ─── START THE CDP TAB MONITOR ────────────────────────────────
    ...
    # ─── START THE DOWNLOAD FOLDER WATCHER ─────────────────────────
    threading.Thread(target=download_folder_watcher, daemon=True).start()

    load_report_history()

    # Remember start URL as last navigated
    LAST_NAVIGATED_URL = START_URL

except Exception:
    # Ensure the error is visible in the log file and then exit
    safe_log(f"FATAL STARTUP ERROR: {traceback.format_exc()}")
    try:
        push_logs()
    except Exception:
        pass
...
# ---------- Main ----------
def main():
    global app_cmd_id, LAST_NAVIGATED_URL
    ...
    try:
        ...
        LAST_NAVIGATED_URL = driver.current_url
    except Exception as e:
